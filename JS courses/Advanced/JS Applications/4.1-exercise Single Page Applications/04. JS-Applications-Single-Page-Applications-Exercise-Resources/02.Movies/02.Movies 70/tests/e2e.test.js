...
const host = 'http://localhost:5500/APPLICATIONS/04.Single-Page-Applications/02.Movies'; // Application host (NOT service host - that can be anything)
...
describe('E2E tests', function () {
  ...
  before(
    async () =>
      (browser = await chromium.launch(
        /*DEBUG ? */{ headless: true, slowMo: 1000}/* : {}*/
      ))
  );
  ...
  describe('Like functionality', async () => {
    ...
    it('Like button should increase total likes by 1 after a click on it', async () => {
      ...
      await page.click('nav >> text=Movies');
      await page.waitForTimeout(interval/*'#movie'*/);
      await page.click(
        `#movie > div div ul li:has-text("${data.title}") >> text=Details`
      );
      await page.waitForSelector('.enrolled-span');
